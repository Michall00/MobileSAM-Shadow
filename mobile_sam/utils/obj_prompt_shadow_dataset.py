...
class ObjPromptShadowDataset(Dataset):
    def __init__(
        ...
        print(
            f"Looking for object masks in {object_masks_dir} and target masks in {target_masks_dir}..."
        )
    ...
    def __getitem__(self, idx: int) -> dict[str, Tensor]:
        ...
        if self.augmenter is not None:
            img, obj_mask, tgt_mask = self.augmenter(
                np.array(img), np.array(obj_mask), np.array(tgt_mask)
            )
