...
from mobile_sam.utils.obj_prompt_shadow_dataset import AugmentationConfig, ObjPromptShadowDataset
...
out_images_dir = Path(
    "/home/msadowski/Studia/Inzynierka/MobileSAM/dataset/prepared_soba/images_aug"
)
out_obj_dir = Path(
    "/home/msadowski/Studia/Inzynierka/MobileSAM/dataset/prepared_soba/object_masks_aug"
)
...
augment_cfgs = [
    (
        "flip",
        AugmentationConfig(
            use_flip=True, use_brightness=False, use_rotate=False, use_blur=False, use_hue=False
        ),
    ),
    (
        "rotate",
        AugmentationConfig(
            use_flip=False, use_brightness=False, use_rotate=True, use_blur=False, use_hue=False
        ),
    ),
    (
        "bright",
        AugmentationConfig(
            use_flip=False, use_brightness=True, use_rotate=False, use_blur=False, use_hue=False
        ),
    ),
    (
        "blur",
        AugmentationConfig(
            use_flip=False, use_brightness=False, use_rotate=False, use_blur=True, use_hue=False
        ),
    ),
    (
        "hue",
        AugmentationConfig(
            use_flip=False, use_brightness=False, use_rotate=False, use_blur=False, use_hue=True
        ),
    ),
]
...
for i in tqdm(range(len(dataset))):
    ...
    for k, (cfg_name, augmenter) in enumerate(augment_cfgs):
        aug_img, aug_obj, aug_tgt = augmenter(
            np.array(img), np.array(obj_np.copy()), np.array(tgt_np.copy())
        )
