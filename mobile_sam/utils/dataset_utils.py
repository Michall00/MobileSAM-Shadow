    ...
    def __init__(self, cfg_aug: dict):
        ...
        if cfg_aug.get("rotate", False):
            self.transforms.append(
                A.Rotate(limit=cfg_aug.get("rotate_limit", 15), border_mode=0, p=p)
            )
        ...
        self.pipeline = A.Compose(
            self.transforms, additional_targets={"obj_mask": "mask", "tgt_mask": "mask"}
        )

    def __call__(
        self, image: Image.Image, obj_mask: np.ndarray, tgt_mask: np.ndarray
