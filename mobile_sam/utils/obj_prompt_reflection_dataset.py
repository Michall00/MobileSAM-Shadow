...
class ObjPromptReflectionDataset(Dataset):
    def __init__(
        ...
        print("Scanning for Reflection dataset...")

        for img_dir, obj_dir, tgt_dir in zip(
            self.images_dir_list, self.object_masks_dir_list, self.target_masks_dir_list
        ):
            img_paths = sorted(p for p in img_dir.rglob("*.*"))
            for ip in img_paths:
                if ip.suffix.lower() not in valid_ext:
                    continue

                base = ip.stem
                op = next(
                    (p for p in [obj_dir / f"{base}.png", obj_dir / f"{base}.jpg"] if p.is_file()),
                    None,
                )
                tp = next(
                    (p for p in [tgt_dir / f"{base}.png", tgt_dir / f"{base}.jpg"] if p.is_file()),
                    None,
                )
    ...
    def __getitem__(self, idx: int) -> dict[str, Tensor]:
        ...
        if self.augmenter is not None:
            img, obj_mask, refl_mask = self.augmenter(
                np.array(img), np.array(obj_mask), np.array(refl_mask)
            )
...
def show_object_prompt_sample(batch: dict, index: int = 0, title_suffix: str = "") -> None:
    ...
    if neg.size > 0:
        ax.scatter(
            neg[:, 0], neg[:, 1], c="red", s=100, marker="x", label="Negative", edgecolors="black"
        )
