...
from mobile_sam.utils.common import make_panel_with_points, sam_denormalize_float
...
class Trainer:
    def __init__(
        ...
        self.optimizer = torch.optim.AdamW(
            params_to_optimize, lr=lr, weight_decay=weight_decay, betas=(0.9, 0.999)
        )
    ...
    def define_params_to_regularize(self) -> None:
        self.params_to_regularize = [
            ...
            if p.requires_grad and "bias" not in name and "norm" not in name and "bn" not in name
        ]
            ...
            for b_idx, batch in enumerate(loader):
                ...
                for i in range(images.size(0)):
                    ...
                    if box.numel() == 4:
                        ...
                        panel = make_panel_with_points(
                            img_np, gt_np, pred_np, points[i].cpu().numpy()
                        )
    ...
    def baseline_evaluation(self) -> None:
        ...
        sparsity = (
            self.custom_sparsity if self.custom_sparsity is not None else self.calculate_sparsity()
        )
        ...
        log.info(
            f"[Baseline Eval] F1: {val_stats['val_f1']:.4f} | Sparsity: {sparsity:.2%} | Loss: {val_stats['val_loss']:.4f} | Global Epoch: {self.epoch_offset}"
        )
    ...
    def train_epoch(self, epoch: int) -> dict[str, float]:
        ...
        for batch in tqdm(self.train_loader, desc=f"Epoch {epoch}", leave=False):
            ...
            masks = batch["mask"].to(self.device, non_blocking=True)
            obj_mask = (
                batch["obj_mask"].to(self.device, non_blocking=True)
            ...
            if self.grad_clip is not None:
                ...
                torch.nn.utils.clip_grad_norm_(
                    self.model.image_encoder.parameters(), self.grad_clip
                )
    ...
    def fit(self, ckpt_path: str) -> None:
        ...
        ckpt_last = (
            ckpt_path.replace(".pt", "_last.pt")
            if ckpt_path.endswith(".pt")
            else ckpt_path + "_last.pt"
        )
        ckpt_best = (
            ckpt_path.replace(".pt", "_best.pt")
            if ckpt_path.endswith(".pt")
            else ckpt_path + "_best.pt"
        )
        ...
        for epoch in tqdm(range(1, self.max_epochs + 1)):
            ...
            sparsity = (
                self.custom_sparsity
                if self.custom_sparsity is not None
                else self.calculate_sparsity()
            )
            ...
            if not math.isnan(val_stats["val_f1"]) and val_stats["val_f1"] > best_f1:
                ...
                if patience_counter >= self.patience:
                    log.info(
                        f"Early Stopping triggered after {epoch} epochs (No improvement for {self.patience} epochs)."
                    )
                    break
